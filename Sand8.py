...
def drawFluids(fluids):
    ...
    for i in range(0,len(fluidParticles)):
        changex=randint(-1,1)
        if screen.get_at((fluidParticles[i][0]-1,fluidParticles[i][1]-1))==(0,0,0,255):
            fluidParticles[i][0]-=1
            fluidParticles[i][1]+=1
        if screen.get_at((fluidParticles[i][0]+1,fluidParticles[i][1]-1))==(0,0,0,255):
            fluidParticles[i][0]+=1
            fluidParticles[i][1]+=1
        '''if screen.get_at((fluidParticles[i][0]+1,fluidParticles[i][1]))==(0,0,0,255):
            fluidParticles[i][0]+=1
        if screen.get_at((fluidParticles[i][0]-1,fluidParticles[i][1]))==(0,0,0,255):
            fluidParticles[i][0]-=1'''
        draw.circle(screen,(70,140,155),(fluidParticles[i][0],fluidParticles[i][1]),2)
while running:
    screen.fill((0,0,0))
    clock=time.Clock()
    clock.tick(100)
    ...
    for i in range(0,len(drawtool)):
        draw.rect(screen,(200,100,100),(drawtool[i][0],drawtool[i][1],10,10))
    if 1 == mb[0]:
        for i in range(0,1):
            spawnFluid(mx,my)
